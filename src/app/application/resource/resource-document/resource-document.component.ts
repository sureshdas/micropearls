...
@Component({
  ...
  styleUrls: ['./resource-document.component.scss']
})
export class ResourceDocumentComponent implements OnInit {
  ...
  files: any[] = [{filePath:'assets/no-image.png', fileType:'png' }];
  isAdd: Boolean = false;
    ...
    this.homeService.currentData.pipe(take(1)).subscribe((message: any) => {
      if (message) {
        ...
        this.resourceService
          .getResourceData(`${message.uniqueId}_Documents`)
          .subscribe((e: any) => {
           console.log('document lib res ::', e);
           if (e.data.length == 0) {
            this.isSave = true;
          } else {
            this.isSave = false;
            console.log("data.media",e.data[0].media);
            this.documentsObj.media= e.data[0].media;
            console.log("data.resourceType",e.data[0].resourceType)
            this.documentsObj.resourceType=e.data[0].resourceType;
            console.log("getting documentsObj",this.documentsObj);
            this.documentsObj.media.unshift({
              filePath: 'assets/no-image.png'
            });
            this.files=this.documentsObj.media;
...
console.log("queryResult",queryResult);
  ...
  handleRendererChange = (ev: any) => {
    ...
    if (!this.uniqueId) {
      ...
      this.upload('addDocumentFile');
    } else {
      // this.updateTechFile(ev, this.uniqueId);
      this.upload('editDocumentFile', this.uniqueId);
    }
  };
...
console.log("file id",fileId);
    ...
    if (fileCount > 0) {
      ...
      if (fileId) {
        const newDocumentsObj = { ...this.documentsObj };
        this.mediaService
          .updateMedia(formData, fileId)
          .pipe(take(1))
          .subscribe((res: any) => {
            console.log("result",res);
            for (let i = 0; i < newDocumentsObj.length; i++) {
              if (newDocumentsObj.files[i].uniqueId === fileId) {
                newDocumentsObj.media[i].filePath = res.filePath;
                this.documentsObj = newDocumentsObj;
                break;
              }
            }
            for (let i = 0; i < this.files.length; i++) {
        ...
        this.mediaService
          ...
          .subscribe((res: any) => {
            ...
            // this.documentsObj.media=[...this.files];
  ...
  updateResource() {
   ...
   k++;
    ...
    this.documentsObj.media=[...array];
    this.homeService.currentData.pipe(take(1)).subscribe((message: any) => {
      if (message) {
        this.Obj.applicationDetailId = message.uniqueId;
      }
    });
    let obj = [{...this.documentsObj}];
    ...
    this.resourceService
      ...
      .subscribe((e: any) => {
        ...
        this.sharedService.showLoader =false;
        this.openSnackBar('Document Library Updated');
  ...
  removeFiles(file: any) {
    ...
    let updatedFiles: any[] = [];
      if (this.documentsObj.media[0].filePath === 'assets/no-image.png') {
        for (let i = 1; i < this.documentsObj.media.length; i++) {
          if (this.documentsObj.media[i].uniqueId === file.uniqueId) {
            this.documentsObj.media.splice(i, 1);
            updatedFiles = this.documentsObj.media;
  ...
  saveResource() {
    ...
    console.log("files",this.files)
    for(let i=1;i<this.files.length;i++){
      this.documentsObj.media[k]=this.files[i];
      k++;
    }
    this.homeService.currentData.pipe(take(1)).subscribe((message: any) => {
      if (message) {
    ...
    console.log("this.documentsObj:::",this.documentsObj)
    let obj:any=[{
      media:[],
      resourceType:'',
  }];
  obj[0].media=this.documentsObj.media;
  obj[0].resourceType=this.documentsObj.resourceType;
  console.log("this.presentationObj:::",this.documentsObj);
    // let obj = [{...this.documentsObj}];
    // if(obj[0].media && obj[0].media[0].filePath === 'assets/no-image.png') {
    //   obj = obj.map((el: any) => {
    //     el.media.shift();
    ...
    this.resourceService
      ...
      .subscribe((e: any) => {
        ...
        this.openSnackBar('Document Library Saved');
      });
  }
